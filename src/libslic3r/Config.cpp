...
ConfigSubstitutions ConfigBase::load_from_ini_string_commented(std::string &&data, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    // Also the "; generated by CaribouSlicer ..." comment line will be removed.
    size_t j = 0;
...
ConfigSubstitutions ConfigBase::load_from_gcode_file(const std::string &filename, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    // Look for Slic3r or CaribouSlicer header.
    ...
    bool has_delimiters = false;
    {
        ...
        static constexpr const char caribouslicer_gcode_header[] = "; generated by CaribouSlicer ";
        std::string header;
        ...
        while (std::getline(ifs, header)) {
            if (strncmp(slic3r_gcode_header, header.c_str(), strlen(slic3r_gcode_header)) == 0) {
                header_found = true;
                break;
            } else if (strncmp(caribouslicer_gcode_header, header.c_str(), strlen(caribouslicer_gcode_header)) == 0) {
                // Parse CaribouSlicer version.
                size_t i = strlen(caribouslicer_gcode_header);
        ...
        if (! header_found)
            throw Slic3r::RuntimeError("Not a CaribouSlicer / Slic3r PE generated g-code.");
    }
    ...
    if (has_delimiters)
    {
        ...
        while (reader.getline(line))
            if (line == "; caribouslicer_config = end") {
                end_found = true;
                break;
            }
        if (! end_found)
            throw Slic3r::RuntimeError(format("Configuration block closing tag \"; caribouslicer_config = end\" not found when reading %1%", filename));
        std::string key, value;
        while (reader.getline(line)) {
            if (line == "; caribouslicer_config = begin") {
                begin_found = true;
                break;
            }
        ...
        if (! begin_found)
            throw Slic3r::RuntimeError(format("Configuration block opening tag \"; caribouslicer_config = begin\" not found when reading %1%", filename));
    }
    else
    {
