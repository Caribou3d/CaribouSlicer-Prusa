...
public:
    ...
    bool load_opengl_dll()
    {
        ...
        wc.lpszClassName = L"CaribouSlicer_opengl_version_check";
        wc.style = CS_OWNDC;
        if (RegisterClass(&wc)) {
            HWND hwnd = CreateWindowW(wc.lpszClassName, L"CaribouSlicer_opengl_version_check", WS_OVERLAPPEDWINDOW, 0, 0, 640, 480, 0, 0, wc.hInstance, (LPVOID)this);
            if (hwnd) {
...
// http://download.qt.io/development_releases/prebuilt/llvmpipe/windows/
    if (load_mesa) {
        ...
        if (!res) {
            MessageBox(nullptr, L"CaribouSlicer was unable to automatically switch to MESA OpenGL library\nPlease, try to run the application using the '--sw-renderer' option.\n",
                L"CaribouSlicer Warning", MB_OK);
            return -1;
        }
        else {
...
//	printf("Loading Slic3r library: %S\n", path_to_slic3r);
    ...
    if (slic3r_main == nullptr) {
        printf("could not locate the function slic3r_main in CaribouSlicer.dll\n");
        return -1;
    }
