///|/ Copyright (c) Prusa Research 2020 - 2023 Oleksandra Iushchenko @YuSanka, David Kocík
///@kocikdav, Vojtěch Bubník @bubnikv, Lukáš Matěna @lukasmatena
///|/
...
PresetForPrinter::PresetForPrinter(PhysicalPrinterDialog *parent, const std::string &preset_name)
    ...
    m_info_line = new wxStaticText(
        parent, wxID_ANY, _L("This printer will be shown in the presets list as") + ":"
    );
...
void PresetForPrinter::DeletePreset(wxEvent &event) { m_parent->DeletePreset(this); }
...
void PresetForPrinter::update_full_printer_name() {
    ...
    m_full_printer_name->SetLabelText(
        printer_name + from_u8(PhysicalPrinter::separator()) + preset_name
    );
}
...
bool save_secret(
    const std::string &id, const std::string &opt, const std::string &usr, const std::string &psswd
) {
#if wxUSE_SECRETSTORE
    wxSecretStore store = wxSecretStore::GetDefault();
    wxString errmsg;
    if (!store.IsOk(&errmsg)) {
        std::string msg = GUI::format(
            "%1% (%2%).", _u8L("This system doesn't support storing passwords securely"), errmsg
        );
    ...
    const wxString service =
        GUI::format_wxstr(L"%1%/PhysicalPrinter/%2%/%3%", SLIC3R_APP_NAME, id, opt);
    ...
    BOOST_LOG_TRIVIAL(
        error
    ) << "wxUSE_SECRETSTORE not supported. Cannot save password to the system store.";
    return false;
...
bool load_secret(const std::string &id, const std::string &opt, std::string &usr, std::string &psswd) {
    ...
    BOOST_LOG_TRIVIAL(
        error
    ) << "wxUSE_SECRETSTORE not supported. Cannot load password from the system store.";
    return false;
#endif // wxUSE_SECRETSTORE
}
} // namespace
...
PhysicalPrinterDialog::PhysicalPrinterDialog(wxWindow *parent, wxString printer_name)
    ...
    wxStaticText *label_top =
        new wxStaticText(this, wxID_ANY, _L("Descriptive name for the printer") + ":");
    ...
    if (!printer) {
        ...
        // if printer_name is empty it means that new printer is created, so enable all items in the
        // preset list
...
void PhysicalPrinterDialog::build_printhost_settings(ConfigOptionsGroup *m_optgroup) {
    ...
    auto create_sizer_with_btn =
        [](wxWindow *parent, ScalableButton **btn, const std::string &icon_name,
           const wxString &label) {
            *btn = new ScalableButton(
                parent, wxID_ANY, icon_name, label, wxDefaultSize, wxDefaultPosition,
    ...
    auto printhost_browse = [=](wxWindow *parent) {
        auto sizer = create_sizer_with_btn(
            parent, &m_printhost_browse_btn, "browse", _L("Browse") + " " + dots
        );
    ...
    auto print_host_printers = [this, create_sizer_with_btn](wxWindow *parent) {
        // add_scaled_button(parent, &m_printhost_port_browse_btn, "browse", _(L("Refresh
        // Printers")), wxBU_LEFT | wxBU_EXACTFIT);
        auto sizer = create_sizer_with_btn(
            parent, &m_printhost_port_browse_btn, "browse", _(L("Refresh Printers"))
        );
    ...
    if (Http::ca_file_supported()) {
        ...
        auto printhost_cafile_browse = [=](wxWindow *parent) {
            auto sizer = create_sizer_with_btn(
                parent, &m_printhost_cafile_browse_btn, "browse", _L("Browse") + " " + dots
            );
            m_printhost_cafile_browse_btn->Bind(wxEVT_BUTTON, [this, m_optgroup](wxCommandEvent e) {
                static const auto filemasks = _L(
                    "Certificate files (*.crt, *.pem)|*.crt;*.pem|All files|*.*"
                );
                wxFileDialog openFileDialog(
                    this, _L("Open CA certificate file"), "", "", filemasks,
            ...
            std::string info = _u8L("HTTPS CA File") + ":\n\t" +
                (boost::format(_u8L(
                 ...
                 SLIC3R_APP_NAME)
                    .str() +
                "\n\t" +
                _u8L("To use a custom CA file, please import your CA file into Certificate Store / "
                     "Keychain.");

            // auto txt = new wxStaticText(parent, wxID_ANY, from_u8((boost::format("%1%\n\n\t%2%")
            // % info % ca_file_hint).str()));
            auto txt = new wxStaticText(
                parent, wxID_ANY, from_u8((boost::format("%1%\n\t%2%") % info % ca_file_hint).str())
            );
    ...
    if (is_secret_store_ok()) {
        ...
        line.widget = [ca_file_hint](wxWindow *parent) {
            wxString info = GUI::format_wxstr(
                ...
                GUI::format_wxstr(
                    _L("On this system, %1% uses the system password store to safely store and "
                       "read passwords and API keys."),
                    SLIC3R_APP_NAME
                )
            );
        ...
        line.widget = [ca_file_hint](wxWindow *parent) {
            wxString info = GUI::format_wxstr(
                L"%1%:\n\t%2%\n\t%3%\n", _L("Storing passwords"),
                GUI::format_wxstr(
                    _L("On this system, %1% cannot access the system password store."), SLIC3R_APP_NAME
                ),
    ...
    for (const std::string &opt_key :
         std::vector<std::string>{"printhost_user", "printhost_password"}) {
    ...
    if (printhost_field) {
        ...
        if (temp) {
            temp->Bind(
                wxEVT_TEXT, ([printhost_field, temp](wxEvent &e) {
#ifndef __WXGTK__
                    e.Skip();
    ...
    if (tech == ptFFF) {
        ...
        if (opt->value == htPrusaLink) { // PrusaConnect does NOT allow http digest
            ...
            AuthorizationType auth_type =
                m_config
                    ->option<ConfigOptionEnum<AuthorizationType>>("printhost_authorization_type")
                    ->value;
        ...
        AuthorizationType auth_type =
            m_config->option<ConfigOptionEnum<AuthorizationType>>("printhost_authorization_type")
                ->value;
        m_optgroup->show_field("printhost_apikey", auth_type == AuthorizationType::atKeyPassword);
...
void PhysicalPrinterDialog::update_host_type(bool printer_change) {
    ...
    for (PresetForPrinter *prstft : m_presets) {
        ...
        if (Preset *preset = wxGetApp().preset_bundle->printers.find_preset(preset_name)) {
            ...
            std::string model_id_no_pref =
                preset->trim_vendor_repo_prefix(model_id, printer_with_vendor.vendor);
            if (printer_with_vendor.vendor) {
                // UPDATED LINE: Check for "Prusa Research" OR "Caribou"
                if (printer_with_vendor.vendor->name.find("Prusa Research") != std::string::npos ||
                    printer_with_vendor.vendor->name.find("Caribou") != std::string::npos) {
                    const std::vector<VendorProfile::PrinterModel> &models =
                        printer_with_vendor.vendor->models;
                    auto it = std::find_if(
    ...
    for (PresetForPrinter *prstft : m_presets) {
        ...
        // UPDATED LINE: Fail if the vendor is not Prusa Research AND not Caribou
        if (!printer_with_vendor.vendor ||
            (printer_with_vendor.vendor->name.find("Prusa Research") == std::string::npos &&
             printer_with_vendor.vendor->name.find("Caribou") == std::string::npos)) {
            connect.supported = false;
            break;
        }
    ...
    wxArrayString types;
    int last_in_conf =
        m_config->option("host_type")->getInt(); //  this is real position in last choice
    ...
    int index_in_choice =
        (printer_change ?
             std::clamp(
                 last_in_conf - ((int) ht->m_opt.enum_def->values().size() - (int) types.size()), 0,
                 (int) ht->m_opt.enum_def->values().size() - 1
             ) :
             last_in_conf);
    ...
    if (link.supported && link.label == _(ht->m_opt.enum_def->label(index_in_choice)))
        ...
        int host_type = std::clamp(
            index_in_choice + ((int) ht->m_opt.enum_def->values().size() - (int) types.size()), 0,
            (int) ht->m_opt.enum_def->values().size() - 1
        );
...
wxString PhysicalPrinterDialog::get_printer_name() { return m_printer_name->GetValue(); }
...
void PhysicalPrinterDialog::update_full_printer_names() {
    ...
    for (size_t i = 0; i < std::strlen(unusable_symbols); i++) {
        ...
        if (pos != std::string::npos) {
            ...
            InfoDialog(
                this, format_wxstr("%1%: \"%2%\" ", _L("Unexpected character"), str),
...
void PhysicalPrinterDialog::OnOK(wxEvent &event) {
    ...
    if (opt->value == htPrusaConnect) {
        ...
        if (printhost_win && printhost_win->GetValue() != GUI::from_u8(sc.connect_url())) {
            InfoDialog msg(
                this, _L("Warning"),
                // TRN: The placeholder expands to https://connect.prusa3d.com. The warning shows
                // when someone select PrusaConnect physical printer, but enters different URL.
                GUI::format(
                    _L("URL of Prusa Connect is different from %1%. Do you want to continue?"),
                    sc.connect_url()
                ),
                true, wxYES_NO
            );
    ...
    if (existing && into_u8(printer_name) != printers.get_selected_printer_name()) {
        wxString msg_text = from_u8(
            (boost::format(_u8L("Printer with name \"%1%\" already exists.")) %
             existing->name /*printer_name*/)
                .str()
        );
        msg_text += "\n" + _L("Replace?");
    ...
    if (!repeat_presets.empty()) {
        ...
        wxString msg_text = format_wxstr(
            _L_PLURAL(
                "Following printer preset is duplicated:%1%"
    ...
    if (!m_printer.config.opt_string("printhost_password").empty()) {
        if (save_secret(
                m_printer.name, "printhost_password", m_printer.config.opt_string("printhost_user"),
    ...
    if (m_printer.preset_names.find(printers.get_selected_printer_preset_name()) ==
        m_printer.preset_names.end()) {
        // select first preset for this printer
        printers.select_printer(m_printer);
        // refresh preset list on Printer Settings Tab
        wxGetApp()
            .get_tab(Preset::TYPE_PRINTER)
            ->select_preset(printers.get_selected_printer_preset_name());
    } else
...
void PhysicalPrinterDialog::DeletePreset(PresetForPrinter *preset_for_printer) {
    if (m_presets.size() == 1) {
        wxString msg_text = _L(
            "It's not possible to delete the last related preset for the printer."
        );
