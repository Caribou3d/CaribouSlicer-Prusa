...
void GLGizmoMeasure::render_debug_dialog()
{
    auto add_feature_data = [this](const SelectedFeatures::Item& item) {
        ...
        add_strings_row_to_table(*m_imgui, "Type", ImGuiWrapper::COL_GREENC_LIGHT, text, ImGui::GetStyleColorVec4(ImGuiCol_Text));
        switch (item.feature->get_type())
        {
        case Measure::SurfaceFeatureType::Point:
        {
            add_strings_row_to_table(*m_imgui, "m_pt1", ImGuiWrapper::COL_GREENC_LIGHT, format_vec3(item.feature->get_point()), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            break;
        }
        case Measure::SurfaceFeatureType::Edge:
        {
            auto [from, to] = item.feature->get_edge();
            add_strings_row_to_table(*m_imgui, "m_pt1", ImGuiWrapper::COL_GREENC_LIGHT, format_vec3(from), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            add_strings_row_to_table(*m_imgui, "m_pt2", ImGuiWrapper::COL_GREENC_LIGHT, format_vec3(to), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            break;
        }
        case Measure::SurfaceFeatureType::Plane:
        {
            auto [idx, normal, origin] = item.feature->get_plane();
            add_strings_row_to_table(*m_imgui, "m_pt1", ImGuiWrapper::COL_GREENC_LIGHT, format_vec3(normal), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            add_strings_row_to_table(*m_imgui, "m_pt2", ImGuiWrapper::COL_GREENC_LIGHT, format_vec3(origin), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            add_strings_row_to_table(*m_imgui, "m_value", ImGuiWrapper::COL_GREENC_LIGHT, format_double(idx), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            break;
        }
        case Measure::SurfaceFeatureType::Circle:
        {
            ...
            add_strings_row_to_table(*m_imgui, "m_pt1", ImGuiWrapper::COL_GREENC_LIGHT, format_vec3(center), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            add_strings_row_to_table(*m_imgui, "m_pt2", ImGuiWrapper::COL_GREENC_LIGHT, format_vec3(normal), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            add_strings_row_to_table(*m_imgui, "m_value", ImGuiWrapper::COL_GREENC_LIGHT, format_double(radius), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            break;
        }
        }
        std::optional<Vec3d> extra_point = item.feature->get_extra_point();
        if (extra_point.has_value())
            add_strings_row_to_table(*m_imgui, "m_pt3", ImGuiWrapper::COL_GREENC_LIGHT, format_vec3(*extra_point), ImGui::GetStyleColorVec4(ImGuiCol_Text));
    };
    ...
    if (ImGui::BeginTable("Mode", 2)) {
        ...
        add_strings_row_to_table(*m_imgui, "Mode", ImGuiWrapper::COL_GREENC_LIGHT, txt, ImGui::GetStyleColorVec4(ImGuiCol_Text));
        ImGui::EndTable();
    }

    ImGui::Separator();
    if (ImGui::BeginTable("Hover", 2)) {
        add_strings_row_to_table(*m_imgui, "Hover id", ImGuiWrapper::COL_GREENC_LIGHT, std::to_string(m_hover_id), ImGui::GetStyleColorVec4(ImGuiCol_Text));
        const std::string txt = m_curr_feature.has_value() ? surface_feature_type_as_string(m_curr_feature->get_type()) : "None";
        add_strings_row_to_table(*m_imgui, "Current feature", ImGuiWrapper::COL_GREENC_LIGHT, txt, ImGui::GetStyleColorVec4(ImGuiCol_Text));
        ImGui::EndTable();
    }
    ...
    if (!m_selected_features.first.feature.has_value() && !m_selected_features.second.feature.has_value())
        ...
        if (m_selected_features.first.feature.has_value()) {
            m_imgui->text_colored(ImGuiWrapper::COL_GREENC_LIGHT, "Selection 1");
        ...
        if (m_selected_features.second.feature.has_value()) {
            m_imgui->text_colored(ImGuiWrapper::COL_GREENC_LIGHT, "Selection 2");
...
void GLGizmoMeasure::on_render_input_window(float x, float y, float bottom_limit)
{
    ...
    if (ImGui::BeginTable("Commands", 2)) {
        ...
        add_row_to_table(
            [this]() {
                m_imgui->text_colored(ImGuiWrapper::COL_GREENC_LIGHT, _u8L("Left mouse button"));
            },
            [this]() {
                std::string text;
                ColorRGBA color;
        ...
        if (m_mode == EMode::FeatureSelection && m_hover_id != -1) {
            add_strings_row_to_table(*m_imgui, "Shift", ImGuiWrapper::COL_GREENC_LIGHT, _u8L("Enable point selection"), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            ++row_count;
        }

        if (m_selected_features.first.feature.has_value()) {
            add_strings_row_to_table(*m_imgui, "Delete", ImGuiWrapper::COL_GREENC_LIGHT, _u8L("Restart selection"), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            ++row_count;
        }
        ...
        if (m_selected_features.first.feature.has_value() || m_selected_features.second.feature.has_value()) {
          add_row_to_table(
            [this]() {
                m_imgui->text_colored(ImGuiWrapper::COL_GREENC_LIGHT, "Esc");
            },
            [this]() {
        ...
        for (unsigned int i = row_count; i < 4; ++i) {
            add_strings_row_to_table(*m_imgui, " ", ImGuiWrapper::COL_GREENC_LIGHT, " ", ImGui::GetStyleColorVec4(ImGuiCol_Text));
        }
    ...
    if (ImGui::BeginTable("Measure", 4)) {
        if (m_selected_features.second.feature.has_value()) {
            ...
            if (measure.angle.has_value()) {
                ...
                add_measure_row_to_table(_u8L("Angle"), ImGuiWrapper::COL_GREENC_LIGHT, format_double(Geometry::rad2deg(measure.angle->angle)) + "°",
            ...
            if (measure.distance_infinite.has_value()) {
                ...
                add_measure_row_to_table(show_strict ? _u8L("Perpendicular distance") : _u8L("Distance"), ImGuiWrapper::COL_GREENC_LIGHT, format_double(distance) + units,
                    ImGui::GetStyleColorVec4(ImGuiCol_Text));
                ++measure_row_count;
                ImGui::PopID();
            }
            if (show_strict) {
                ...
                add_measure_row_to_table(_u8L("Direct distance"), ImGuiWrapper::COL_GREENC_LIGHT, format_double(distance) + units,
                    ImGui::GetStyleColorVec4(ImGuiCol_Text));
                ++measure_row_count;
                ImGui::PopID();
            }
            if (measure.distance_xyz.has_value() && measure.distance_xyz->norm() > EPSILON) {
                ...
                add_measure_row_to_table(_u8L("Distance XYZ"), ImGuiWrapper::COL_GREENC_LIGHT, format_vec3(distance),
                    ImGui::GetStyleColorVec4(ImGuiCol_Text));
                ++measure_row_count;
                ImGui::PopID();
            }
